...
DROP POLICY IF EXISTS "Anyone can read admin advice" ON admin_advice;
CREATE POLICY "Anyone can read admin advice" ON admin_advice
  FOR SELECT USING (true);

-- Only admins can insert, update, delete admin advice
DROP POLICY IF EXISTS "Only admins can manage admin advice" ON admin_advice;
...
DROP POLICY IF EXISTS "Admins can read all support requests" ON support_requests;
...
DROP TRIGGER IF EXISTS update_admin_advice_updated_at ON admin_advice;
