...
DROP POLICY IF EXISTS "Students can view their quiz results" ON public.quiz_question_results;
...
DROP POLICY IF EXISTS "Admins can view all quiz results" ON public.quiz_question_results;
...
DROP POLICY IF EXISTS "Students can view their video activity" ON public.video_activity_logs;
CREATE POLICY "Students can view their video activity" 
ON public.video_activity_logs 
FOR SELECT 
USING (student_id = auth.uid());

DROP POLICY IF EXISTS "Students can insert their video activity" ON public.video_activity_logs;
CREATE POLICY "Students can insert their video activity" 
ON public.video_activity_logs 
FOR INSERT 
WITH CHECK (student_id = auth.uid());

DROP POLICY IF EXISTS "Admins can view all video activity" ON public.video_activity_logs;
...
DROP POLICY IF EXISTS "Students can view their exam activity" ON public.exam_activity_logs;
CREATE POLICY "Students can view their exam activity" 
ON public.exam_activity_logs 
FOR SELECT 
USING (student_id = auth.uid());

DROP POLICY IF EXISTS "Students can insert their exam activity" ON public.exam_activity_logs;
CREATE POLICY "Students can insert their exam activity" 
ON public.exam_activity_logs 
FOR INSERT 
WITH CHECK (student_id = auth.uid());

DROP POLICY IF EXISTS "Admins can view all exam activity" ON public.exam_activity_logs;
...
DROP POLICY IF EXISTS "Students can view their questions" ON public.student_questions_log;
CREATE POLICY "Students can view their questions" 
ON public.student_questions_log 
FOR SELECT 
USING (student_id = auth.uid());

DROP POLICY IF EXISTS "Students can insert their questions" ON public.student_questions_log;
CREATE POLICY "Students can insert their questions" 
ON public.student_questions_log 
FOR INSERT 
WITH CHECK (student_id = auth.uid());

DROP POLICY IF EXISTS "Students can update their questions" ON public.student_questions_log;
CREATE POLICY "Students can update their questions" 
ON public.student_questions_log 
FOR UPDATE 
USING (student_id = auth.uid());

DROP POLICY IF EXISTS "Admins can view all student questions" ON public.student_questions_log;
...
CREATE INDEX IF NOT EXISTS idx_quiz_question_results_student_id ON public.quiz_question_results(student_id);
CREATE INDEX IF NOT EXISTS idx_quiz_question_results_quiz_attempt ON public.quiz_question_results(quiz_attempt_id);
CREATE INDEX IF NOT EXISTS idx_video_activity_student_id ON public.video_activity_logs(student_id);
CREATE INDEX IF NOT EXISTS idx_video_activity_video_id ON public.video_activity_logs(video_id);
CREATE INDEX IF NOT EXISTS idx_exam_activity_student_id ON public.exam_activity_logs(student_id);
CREATE INDEX IF NOT EXISTS idx_exam_activity_exam_id ON public.exam_activity_logs(exam_id);
CREATE INDEX IF NOT EXISTS idx_student_questions_student_id ON public.student_questions_log(student_id);
CREATE INDEX IF NOT EXISTS idx_student_questions_topic ON public.student_questions_log(topic);
...
DROP TRIGGER IF EXISTS update_student_questions_log_updated_at ON public.student_questions_log;
